...
import com.zmops.zeus.iot.server.transfer.core.db.LocalProfile;
...
import static com.zmops.zeus.iot.server.transfer.conf.AgentConstants.AGENT_CONF_PARENT;
import static com.zmops.zeus.iot.server.transfer.conf.AgentConstants.DEFAULT_AGENT_CONF_PARENT;
...
public class AgentManager extends AbstractDaemon {
    ...
    public AgentManager() {
        ...
        if (conf.getBoolean(AgentConstants.AGENT_ENABLE_HTTP, AgentConstants.DEFAULT_AGENT_ENABLE_HTTP)) {
    ...
    private ProfileFetcher initFetcher(AgentManager agentManager) {
        try {
            Constructor<?> constructor = Class.forName(conf.get(AgentConstants.AGENT_FETCHER_CLASSNAME)).getDeclaredConstructor(AgentManager.class);
    ...
    private Db initDb() {
        try {
            ...
            return (Db) Class.forName(conf.get(AgentConstants.AGENT_DB_CLASSNAME, AgentConstants.DEFAULT_AGENT_DB_CLASSNAME)).newInstance();
        } catch (Exception ex) {
